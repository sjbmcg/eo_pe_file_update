#!/usr/bin/env python3
"""
Sprite Sheet Slicer snd update the egf file
...
def update_pe_with_bitmaps(pe_path: str, dibs: list[bytes], start_id: int):
    ...
    try:
        ...
        for i, dib in enumerate(dibs):
            name_id = MAKEINTRESOURCE(start_id + i)
...
def main():
    ...
    # Calculate first_count if not provided
    ...
    if to1:
        update_pe_with_bitmaps(args.egf1_file, to1, next1)
        with open(args.egf1_file, 'rb') as f:
            with open(args.output1, 'wb') as out:
                out.write(f.read())

    if to2:
        update_pe_with_bitmaps(args.egf2_file, to2, next2)
